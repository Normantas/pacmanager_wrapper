...
pub fn get_command(action: PacManagerAction) -> String {
    ...
    pacmanager_command = format!("{pacmanager_command}yum");
    pacmanager_command = format!("{pacmanager_command} {action_command}");
    if action.non_interactive {
        pacmanager_command = format!("{pacmanager_command} -y");
    }
...
mod tests {
    ...
    fn remove_package() {
        let dummy_action = PacManagerAction {
            ...
            internal_config: Default::default(),
            non_interactive: true,
            custom_flags: Some(vec!["--color=always".to_string()]),
        };

        assert_eq!(
            "sudo yum remove Test -y --color=always",
            get_command(dummy_action)
        );
    }
    ...
    fn update_system_packages() {
        ...
        assert_eq!("sudo yum upgrade -y", get_command(dummy_action));
    }
}
